...
## SHA-256 for reset tokens instead of bcrypt
Passwords are hashed with bcrypt because they're low-entropy and
human-chosen, so deliberate slowness matters against brute-forcing.
Reset tokens are 32 random bytes with far higher entropy than any
password, generated by us, not chosen by a human — a fast hash
(SHA-256) is appropriate and sufficient here, and using bcrypt would
add unnecessary slowness with no real security benefit.

## Reset link via a URL query parameter instead of adding a routing library
This project has deliberately never added react-router. Rather than
introduce it just for one reset-password screen, the link uses a
plain ?resetToken= query parameter that App.jsx checks once on mount
— consistent with keeping dependencies minimal until genuinely needed.

## Identical response for existing vs. non-existing email on forgot-password
Prevents using the endpoint to discover which emails have accounts,
the same principle as the identical login error message from Step 21.
One known, minor gap left honestly unfixed: because sending the email
only happens when the account exists, response *timing* could
theoretically still leak that distinction to a very determined
attacker — a fully constant-time response was judged out of scope for
this project.